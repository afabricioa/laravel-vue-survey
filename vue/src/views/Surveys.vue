<script setup>
    import { RouterLink } from 'vue-router';
    import PageComponent from '../components/PageComponent.vue';
    import { useUserStore } from '../store/index';
    import { watch, computed } from 'vue';

    const store = useUserStore();

    let surveys = computed(() => store.surveys.data);
    let surveysLoading = computed(() => store.surveys.loading);
    store.getSurveys();

    function deleteSurvey(survey){
        if(confirm(`Are you sure you want to delete this survey?`)){
            store.deleteSurvey(survey.id).then(() => {
                store.getSurveys();
            });
        }
    }

</script>

<template>
    <PageComponent>
        <template v-slot:header>
            <div class="flex justify-between items-center">
                <h1 class="text-3x1 font-bold text-gray-900">Surveys</h1>
                <RouterLink to="/surveys/create"
                    class="py-2 px-3 text-white bg-emerald-500 rounded-md hover:bg-emerald-600"
                >
                    Add New Survey
                </RouterLink>
            </div>
        </template>
        <div v-if="surveysLoading" class="flex justify-center">Loading...</div>
        <div v-else class="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
            <div
                v-for="survey in surveys"
                class="flex flex-col py-4 px-6 shadow-md bg-white hover:bg-gray-50 h-[470px]"
            >
                <img :src="survey.image" alt="" class="w-full h-48 object-cover"/>
                <h4 class="mt-4 text-lg font-bold">{{ survey.title }}</h4>
                <div v-html="survey.description" class="overflow-hidden flex-1"></div>

                <div class="flex justify-between">
                    <div class="flex justify-between items-center mt-3">
                        <router-link
                            :to="{name: 'SurveyView', params: {id: survey.id}}"
                            class="py-2 px-4 bg-gray-600 text-white rounded-md hover:bg-gray-400"
                        >
                            Open
                        </router-link>
                    </div>
                    <button
                        v-if="survey.id"
                        type="button"
                        @click="deleteSurvey(survey)"
                        class="h-8 w-8 mt-4 flex items-center justify-center
                            rounded-full border border-transparent
                            text-sm text-red-500
                        "
                    >
                        <svg
                            xmlns="http://www.w3.org/2000/svg"
                            fill="none"
                            viewBox="0 0 24 24"
                            stroke-width="1.5"
                            stroke="currentColor"
                            class="w-6 h-6"
                        >
                            <path
                                stroke-linecap="round"
                                stroke-linejoin="round"
                                d="M14.74 9l-.346 9m-4.788 0L9.26 9m9.968-3.21c.342.052.682.107 1.022.166m-1.022-.165L18.16 19.673a2.25 2.25 0 01-2.244 2.077H8.084a2.25 2.25 0 01-2.244-2.077L4.772 5.79m14.456 0a48.108 48.108 0 00-3.478-.397m-12 .562c.34-.059.68-.114 1.022-.165m0 0a48.11 48.11 0 013.478-.397m7.5 0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 51.964 0 00-3.32 0c-1.18.037-2.09 1.022-2.09 2.201v.916m7.5 0a48.667 48.667 0 00-7.5 0" />
                        </svg>
                    </button>
                </div>
            </div>
        </div>
    </PageComponent>
</template>

